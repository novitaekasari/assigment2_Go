...
const (
	...
	DeletedOrderByIdQuery = `
	DELETED FROM orders WHERE order_id = $1
	`
)
...
// DeletedOrder implements order_repository.Repository.
func (orderPg *orderPG) DeletedOrder(orderId int) errs.Error {

	tx, err := orderPg.db.Begin()

	if err != nil {
		return  errs.NewInternalServerError("something want wrong")
		}
	_, err = tx.Exec(DeletedOrderByIdQuery, orderId)
